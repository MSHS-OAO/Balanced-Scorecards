...
process_finance_access_data <- function(data, updated_user){
  ...
  data  <- data%>%
    select(FDIV,
           ...
           `YTD Bud Minus Act`,
           `Time Period`)%>%
    mutate(FDIV = as.character(FDIV),
           SITE = as.character(SITE),
           Name = as.character(Name),
           EXPTYPE = as.character(EXPTYPE),
           `Sub Account` = as.character(`Sub Account`),
           `Sub Account Description` = as.character(`Sub Account Description`),
           `Month Budget` = as.numeric(`Month Budget`),
           `YTD Budget` = as.numeric( `YTD Budget`),
           `Month Actual` = as.numeric(`Month Actual`),
           `YTD Actual` = as.numeric(`YTD Actual`),
           `Annual Budget` = as.numeric(`Annual Budget`),
           `Remaining Budget YTD` = as.numeric(`Remaining Budget YTD`),
           `Mo Bud Minus Act` = as.numeric(`Mo Bud Minus Act`),
           `YTD Bud Minus Act` = as.numeric(`YTD Bud Minus Act`),
