...
overtime_file_processs_new_analyze <- function(data, updated_user){
  ...
  non_eni_data <- rbind(mapped_non_eni_data_cc_7,mapped_non_eni_data_cc_4)%>%
    filter(!(is.na(`Discharge Fisc Year-Period`)),
           ...
           Site = "MSW") %>%
    select(-VP,-`FLSA Category`, -`Uncontollable POT flag`,-`Pay Category`,-COFT,-CostCenter,-`Location Code`,-`Job Title`,-CORPORATE.SERVICE.LINE,-CostCenterDesc)  %>%
    rename(Site = SITE,
  ...
  processed_data <- non_eni_data %>%distinct()
...
updated_user <- "TEST"
processed_data <- overtime_file_processs_new_analyze(raw_data,updated_user)
