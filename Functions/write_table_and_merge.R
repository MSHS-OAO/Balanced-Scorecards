...
write_temporary_table_to_database_and_merge <- function(processed_input_data,table_name, button_name){
  if(nrow(processed_input_data) == 0) {
    ...
    processed_input_data <- processed_input_data %>% filter(SITE %in% c("MSBI", "MSQ", "MSH", "MSW", "MSB", "NYEE", "MSM", "SYSTEM", "System", "ISM")) # SYSTEM comes from premier while System comes from Finance
