...
write_temporary_table_to_database_and_merge <- function(processed_input_data,table_name, button_name){
  if(nrow(processed_input_data) == 0) {
    ...
    productivity_sites <- unique(processed_input_data$SITE) 
    DEST_TABLE <- if("Worked Hours Productivity Index" %in% productivity_check & "SYSTEM" %in% productivity_sites){
    ...
    processed_input_data <- processed_input_data %>% filter(SITE %in% c("MSBI", "MSQ", "MSH", "MSW", "MSB", "NYEE", "MSM","MSBHC", "SYSTEM"))
