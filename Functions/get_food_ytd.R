get_food_ytd <- function(month_input) {
  ...
  ytd_metrics <- sr_tbl %>% filter(SERVICE %in% 'Food Services',
                                   TO_DATE(min_month, format) <= REPORTING_MONTH) %>%
    select(-UPDATED_TIME, -UPDATED_USER) %>% collect() %>%
    filter(METRIC_NAME_SUBMITTED %in% c('Net Cost of Case per Patient Day (YTD)')) %>%
    ...
    mutate(Reporting_Month = format(Reporting_Month_Ref, "%m-%Y"),
           ...
           Metric_Name =  case_when(
             METRIC_NAME_SUBMITTED == "Net Cost of Case per Patient Day (YTD)" ~ "Net Cost of Case per Patient Day",
