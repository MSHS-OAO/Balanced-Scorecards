...
get_values_system_wide <- function(x,table_name){
  
  service <- x[1]
  month <- x[2]
  premier_reporting_period <- x[3]
  metric_name_submitted <- x[4]
  value <- x[5]
  update_time <- x[6]
  updated_user <- x[7]
  
  values <- glue("INTO \"{table_name}\" (SERVICE,REPORTING_MONTH,METRIC_NAME_SUBMITTED,VALUE,UPDATED_TIME,PREMIER_REPORTING_PERIOD,UPDATED_USER) 
                 VALUES('{service}',TO_DATE('{month}','YYYY-MM-DD'),'{metric_name_submitted}',{value},TO_TIMESTAMP('{update_time}','YYYY-MM-DD HH24:MI:SS'),'{premier_reporting_period}','{updated_user}')")
...
write_temporary_table_to_database_and_merge_system_wide <- function(processed_input_data,table_name, button_name){
  if(nrow(processed_input_data) == 0) {
    ...
    TABLE_NAME <- paste0("BSC_SYSTEM_WIDE_STAGING")
    ...
    inserts <- lapply(
      lapply(
        lapply(split(processed_input_data , 
               ...
               as.list), 
        as.character),
      FUN = get_values_system_wide ,TABLE_NAME)
    ...
    # seq_along : https://r-lang.com/seq_along-function-in-r/
    ...
    query = glue('MERGE INTO BSC_SYSTEM_WIDE_PRODUCTIVITY_FINANCE SR
                    USING "{TABLE_NAME}" SOURCE_TABLE
                    ON (  SR."REPORTING_MONTH" = SOURCE_TABLE."REPORTING_MONTH" AND
                    ...
                    VALUES( SOURCE_TABLE."REPORTING_MONTH",
