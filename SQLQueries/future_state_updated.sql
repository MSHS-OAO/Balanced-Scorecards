...
-- YTD Actual Annualized
    ...
    (
            ...
            WHERE EXPTYPE IN ('Salaries', 'Supplies') --AND Function = 'Support Services' AND TO_DATE(MONTH) = TO_DATE((Select max(MONTH)FROM BSC_FINANCE_TABLE_VIEW))
    ...
    (
            ...
            FROM BSC_FINANCE_TABLE_VIEW
            WHERE EXPTYPE IN ('Salaries', 'Supplies') AND TO_DATE(MONTH) >= TO_DATE((Select add_months(max(MONTH), -11) FROM BSC_FINANCE_TABLE_VIEW))
    ...
    WHERE c.MONTH = TO_DATE((Select max(MONTH) FROM BSC_FINANCE_TABLE_VIEW))
...
SELECT * FROM(
    SELECT b.FUNCTION, b.MONTH, b.EXPTYPE, a.YTD_ACTUAL+b.SUM_OF_REMAINING_BUDGET_YTD AS PROSPECTIVE_OUTLOOK FROM(
        ...
        RIGHT JOIN
        (SELECT FUNCTION, MONTH, EXPTYPE, sum(SUM_OF_REMAINING_BUDGET_YTD) AS SUM_OF_REMAINING_BUDGET_YTD FROM BSC_FINANCE_TABLE_VIEW WHERE EXPTYPE IN ('Salaries', 'Supplies') AND Month = TO_DATE((SELECT max(MONTH) FROM BSC_FINANCE_TABLE_VIEW))
